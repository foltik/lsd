...
pub fn register_routes(router: AppRouter) -> AppRouter {
    router
        ...
        .route("/post/new", get(create_post_page))
        .route("/post/{url}", get(view_post_page))
        .route("/post/{url}/edit", get(edit_post_page).post(edit_post_form))
        .route("/post/{url}/send", get(send_post_page).post(send_post_form))
        .route("/post/{url}/delete", post(delete_post_form))
}
...
async fn edit_post_form(
    ...
    Ok(Redirect::to(&format!("{}/post/{}", state.config.app.url, &form.post.url)).into_response())
}
